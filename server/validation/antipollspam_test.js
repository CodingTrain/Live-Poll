...
async function testInstantDoubleRegister() {
  ...
  if (check(id) == false) return "test_instantDoubleRegister: fail 1";

  // # Guard false positive.
  if (check(id) == true) return "test_instantDoubleRegister: fail 2";
...
function test() {
  // # Collection.
  const funcs = [testFlushing, testInstantDoubleRegister];
  // # Run & print.
  funcs.forEach((f) => {
    f().then((r) => {
      console.log(r);
